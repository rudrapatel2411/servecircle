    ...
    <div className="page-content" style={{ minHeight: '92vh' }}>
      ...
      <div className="services-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '20px' }}>
        {categoryServices.map((service) => (
          <Link key={service.id} to={`/customer/services/home-repairs/${service.id}`} className="service-card hover-lift" style={{ '--card-accent': '#3b82f6', display: 'flex', flexDirection: 'column', height: '100%', textDecoration: 'none' }}>
            <div className="service-card-accent" style={{ background: '#3b82f6', display: 'none' }} />
            <div className="service-card-body" style={{ 
              ...
              <div style={{ borderTop: '1px solid rgba(255,255,255,0.15)', paddingTop: '16px', marginTop: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <span style={{ fontSize: '0.8rem', fontWeight: 800, color: 'white', background: '#3b82f6', padding: '6px 12px', borderRadius: '6px', display: 'flex', alignItems: 'center', gap: '4px' }}>Book now →</span>
              </div>
            </div>
          </Link>
        ))}
